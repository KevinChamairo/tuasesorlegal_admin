  ...
  <div style="overflow-y: scroll;">
    <el-row>
      ...
      >              <div class="btn-back" v-if="isMobile" @click="selectDetail=null"><i class="el-icon-back"></i></div>
        ...
        <lista-abogados v-if="!getInfo && selectDetail " :idAsesoria="id_asesoria" @enviar="enviar"   />
        <div
          v-if="!variable.listAsesoria && !selectDetail && !getInfo"
          class="not-data-div"
        >
          <p class="not-data">
...
import ListaAbogados from "./ListaAbogados"
export default {
  components: {
    ListaAbogados,
    Bandeja,
  ...
  methods: {
    getAsesorias() {
      this.loading = true;
      const endpoint = "/getAsesoria/" + 0 + "/" + 0 + "/" + 0 + "/" + 23
    ...
    detailAsesoria(id_asesoria) {
      this.loading = true
      const endpoint =
        "/getAsesoria/" + id_asesoria + "/" + 0 + "/" + 0 + "/" + 23
      this.$http.get(endpoint).then((response) => {
    ...
    mostrar(val) {
      ...
      this.cambiarEstado.id_asesoria = this.id_asesoria
    ...
    enviar(val) {
      ...
      this.loading = true
      const endpoint = "/crearAsesoriaAbogado"
      this.$http.post(endpoint, val).then((response) => {
        if (response) {
    ...
    this.$message.error("Porfavor, asigne una asesoria", {
      duration: 5000
        });
    }
